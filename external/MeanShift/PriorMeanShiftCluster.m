function [clustCent,data2cluster,clusterVotes,cluster2dataCell] = PriorMeanShiftCluster(dataPts,bandWidth,centres,plotFlag)
%perform MeanShift Clustering of data using a flat kernel
%
% ---INPUT---
% dataPts           - input data, (numDim x numPts)
% bandWidth         - is bandwidth parameter (scalar)
% centres -> prior on where the clusters should be initialised. CHINERY
% ---OUTPUT---
...
if nargin < 3
    ...
    if(size(centres,1) ~= size(dataPts,1))
        error('Prior points must be of same dimension as data');
    end
end
...
if bandWidth <= 0
    error('bandwidth is zero or negative');
end
...
centix = 0;
loop = 1;
myMean      = mean(dataPts,2);

while numInitPts

    if(centix < size(centres,2))
        centix = centix + 1;
        myMean = centres(:,centix);
    else
%         tempInd         = ceil( (numInitPts-1e-6)*rand);        %pick a random seed point
        dists = vnorm(bsxfun(@minus,dataPts(:,initPtInds),myMean)); %pick furthest point away
        [~,tempInd] = max(dists);
    ...
    while loop     %loop untill convergence
        loop = loop +1;
%         sqDistToAll = sum((repmat(myMean,1,numPts) - dataPts).^2);    %dist squared from mean to all points still active
        sqDistToAll = sum(bsxfun(@minus,myMean,dataPts).^2);            % fast version of the above
%         sqDistToAll = exp(1/bandSq*vnorm(bsxfun(@minus,myMean,dataPts)).^2);    % CHINERY: make it gaussian instead
        ...
        % chinery:
        % if mean is NaNs then there aren't any points for this cluster so move on!
        if(any(isnan(myMean)))
            break;
        end
...
% CHINERY return the votes eh?
if nargout > 2
    clusterVotes = bsxfun(@rdivide,double(clusterVotes),sum(clusterVotes,1));
end
