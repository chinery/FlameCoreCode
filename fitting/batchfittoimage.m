...
imgdir = dir([data_root 'images/*.png']);

for frame = 201:length(imgdir)
    twodflame.normm = [];
    twodflame.norms = [];
    twodflame.scale = [];
    twodflame.unifo = [];
    ...
    img = double(imread([data_root 'images/' imgdir(frame).name]))./255;
...
%     [v2, ~] = eig(cov(points(:,points(2,:)<(mean(points(2,:))))'));
    v2 = eye(2);
    rpoints = v2'*points;
    [x,y] = meshgrid(min(rpoints(1,:)):(nn/10):max(rpoints(1,:)),(min(rpoints(2,:))-2*nn):nn:(min(rpoints(2,:)+10*nn)));
    qlrpoints = [repmat(rpoints,1,1) repmat([x(:)';y(:)'],1,5)];%part;%points(:,ix==i | ix==max(ix));
    lrgpoints = v2*qlrpoints;
    lrgpoints(2,:) = round(lrgpoints(2,:)./nn).*nn;
    
    figure(3); hold on; 
    plot(lrgpoints(1,:),lrgpoints(2,:),'.r');

    [a,~] = count_unique(qpoints(2,:));
    core = zeros(2,length(a));
    core(2,:) = a';
    core(1,:) = arrayfun(@(x)mean(lrgpoints(1,lrgpoints(2,:)==x)),unique(qpoints(2,:)));
    ...
    figure(1);clf;displaysingleflame(twodflame);
    view(-20,0);
    save(sprintf([data_root 'flames/frame%05i.mat'],frame),'twodflame');
    print('-dpng',sprintf([data_root 'flames/img/frame%05i'],frame));
end