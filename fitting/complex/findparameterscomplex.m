% this is for finding the parameters of flames once the cores have been found,
% for complex flames (e.g. more than one core per flame)
% it's very similar to findparameters.m in the parent folder, but 
% necessary lines have been uncommented 
...
% allweib = [];
% allgaus = [];
% load(sprintf([data_root 'nmpiotherflames/frame%05i.mat'],333),'flame')
% lastflame = flame;
...
startix = 31; % this is to renumber the flames
global iii
iii = 1;
...
%     clusterix = ones(1,length(points));
    ...
    load([data_root 'othercores/' sprintf('frame%05i.mat',frame)]);
    ...
    for i = 1:length(clustrange)
        ...
        % prepare core
        defival = 20;
        if(~isfield(flame,'cores') || i > length(flame.cores))
            continue;
        end
...
%         ival = defival;
        ...
        sf = 25; %scale factor used when converting to points
        dens = b./sf; 
...
%         end % of col loop
    ...
    save(sprintf([data_root 'otherflames/frame%05i.mat'],frame-startix+1),'flame')
%     lastflame = flame;
% %     
    figure(1);clf;
    ...
    camproj('perspective')
    view(80,0);
    saveas(gcf,sprintf([data_root 'otherflames/img/frame%05i.png'],frame-startix+1));
