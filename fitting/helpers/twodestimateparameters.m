function p = twodestimateparameters(xdata,ydata,prior,tight,col,num)
...
%     upper = [mest*2 mest 1 0.5 1];
% end
% uest = sest/min(ydata(ydata~=0));
uest = 0.01;


weights = ones(1,length(bins))*0.1;
weights(1:length(ydata)) = linspace(2,1,length(ydata));
...
%     modelfun = @(mu,sig,u,s,x)(rotandsum(s.*(... %TG%
%     (1-u).*(...
%         exp(-((x-mu).^2)/(2*sig^2)))...
%     + u)));
...
%     beta0 = [0.5 1 ydata(1) 0.1]; 
    ...
    l = lower([1 2 3 5]);
    u = upper([1 2 3 5]);
% figure(10);clf;plot(linspace(0,0.2,length(ydata)),ydata,'r.');axis(axis);hold on;
...
%     + step(m,u,x)));
% 
% beta0 = [muest sigest uest mest sest];

modelfun = @(mu,sig,u,m,s,x)(rotandsum(s.*(...%TG%
    (step(m,1-u,x)).*(...
        exp(-((x-mu).^2)./(2*sig.^2)))...
    + step(m,u,x))));
...
% % %     upper = [mest*2 mest 1 0.5 1];
% % % end
% figure(10);clf;plot(ydata);axis(axis);hold on;
